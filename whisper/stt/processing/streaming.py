...
async def wssDecode(ws: WebSocketServerProtocol, model_and_alignementmodel):
    ...
    online = OnlineASRProcessor(asr, logfile=sys.stderr, buffer_trimming=8)
...
class OnlineASRProcessor:
    ...
    def __init__(self, asr, buffer_trimming=15, logfile=sys.stderr):
...
class WhisperTimestampedASR(ASRBase):
    ...
    def transcribe(self, audio, init_prompt=""):
        # result = whisper_timestamped.transcribe(self.model, audio, language=self.original_language, **self.transcribe_kargs)
        result = whisper_timestamped.transcribe_timestamped(self.model,
