...
import requests
from linto_stt.backends.whisper.stt import USE_TORCH, USE_TORCHAUDIO, logger
                ...
                if use_mask:
                    logits.append(
                        model(
                            ...
                            attention_mask=padded_batch.attention_mask[:, i:j].to(
                                device),
                        ).logits
                    )
                else:
                    logits.append(
                        model(padded_batch.input_values[:, i:j].to(device)).logits)
