...
import os
import shutil
from linto_stt.backends.whisper.stt import logger, USE_CTRANSLATE2
...
def remove_non_speech(
    ...
    segments = apply_dilatation(
        segments, dilatation, sample_rate, audio, output_sample=True)
    ...
    audio_speech = np.concatenate([audio[..., s:e]
                                  for s, e in segments], axis=-1)
...
def do_convert_timestamps(segments, t, t2=None):
        ...
        if t_in or t2_in:
            result.append(
                [
                    ...
                    max(istart, min(iend, ioffset + t2)
    ...
    if len(result) > 1:
        ...
        result = sorted(result, key=lambda x: abs(
            abs(t2 - t) - abs(x[1] - x[0])))
    result = result[0]
    if t2 is None:
    ...
    if isinstance(method, list):
        ...
        if _silero_vad_model.get(version) is None:
            ...
            repo_or_dir_specific = (
                os.path.expanduser(
                    torch_home + f"/hub/snakers4_silero-vad_{version}")
                if version
        ...
        if auditok.__version__ < "0.3.0":
            segments = [
                {"start": s._meta.start * sample_rate,
                    "end": s._meta.end * sample_rate}
                for s in segments
            ]
        else:
            segments = [
    ...
    if method in [True, "True", "true"]:
        ...
        if method != "silero":
            assert method.startswith(
                "silero:"), f"Got unexpected VAD method {method}"
