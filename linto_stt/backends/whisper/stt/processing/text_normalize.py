...
from linto_stt.backends.whisper.stt import logger
...
def remove_punctuation(text: str, ensure_no_spaces_in_words: bool = False) -> str:
    ...
    if ensure_no_spaces_in_words and " " in new_text:
        ...
        assert not re.search(
            r"[^\W\d\'\-_]", tail), f"Got unexpected word containing space: {text}"
    ...
    for digit in digits:
        ...
        if numslash == 0:
            ...
            if is_date:
                ...
                second = _int_to_month.get(
                    lang, {}).get(second, digitf[i + 1:])
            else:
