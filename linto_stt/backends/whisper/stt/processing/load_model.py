...
from linto_stt.backends.whisper.stt import USE_CTRANSLATE2, logger
        ...
        if not os.path.isfile(os.path.join(model_type_or_file, "model.bin")) and model_type_or_file not in faster_whisper.utils.available_models():
            ...
            if not os.path.isdir(output_dir):
                ...
                kwargs = dict(cache_dir=download_root,
                              use_auth_token=None, revision=None)
                ...
                if not os.path.isdir(model_type_or_file):
                    ...
                    for candidate in ["pytorch_model.bin", "model.safetensors", "whisper.ckpt", "pytorch_model.bin.index.json", "model.safetensors.index.json"]:
                        try:
                            hf_path = model_path = cached_file(
                                model_type_or_file, candidate, **kwargs)
                        except OSError:
                            continue
                        if candidate.endswith("index.json"):
                            ...
                            model_path = list(
                                set(mapping["weight_map"].values()))
                            folder = os.path.dirname(index_file)
                            model_path = [os.path.join(
                                folder, p) for p in model_path]
                        break
                    if model_path is None:
                        raise RuntimeError(
                            f"Could not find model {model_type_or_file} from HuggingFace nor local folders.")
                    hf_path = os.path.dirname(
                        os.path.dirname(os.path.dirname(hf_path)))
                    delete_hf_path = not os.path.exists(hf_path)
                else:
                    hf_path = None
                    for candidate in ["pytorch_model.bin", "model.safetensors", "whisper.ckpt", "pytorch_model.bin.index.json", "model.safetensors.index.json"]:
                        model_path = os.path.join(
                            model_type_or_file, candidate)
                ...
                try:
                    ...
                    logger.info(
                        f"Converting {model_type_or_file} to {output_dir} with:\n{' '.join(cmd)}")
                ...
                assert os.path.isdir(
                    output_dir), f"Failed to build {output_dir}"
            ...
            try:
                model = faster_whisper.WhisperModel(
                    ...
                    download_root=os.path.join(
                        download_root, f"huggingface/hub"),
                )
                logger.info(
                    f"Whisper model loaded with compute_type={compute_type}. (t={time.time() - start}s)")
                break
            except ValueError as err:
                logger.info(
                    "WARNING: failed to load model with compute_type={}".format(
                        compute_type)
                )
...
def check_torch_installed():
    try:
        ...
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "transformers[torch]>=4.23"])
