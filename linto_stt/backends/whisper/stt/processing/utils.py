...
import numpy as np
import wavio
from linto_stt.backends.whisper.stt import USE_CTRANSLATE2, USE_TORCH, USE_TORCHAUDIO
...
def get_device():
    ...
    if USE_CTRANSLATE2:
        ...
        try:
            ...
            raise Exception(
                "Failed to set device: {}".format(str(err))) from err
...
def get_language(language=None):
    ...
    if isinstance(language, str) and language not in LANGUAGES:
        candidate_language = {v: k for k,
                              v in LANGUAGES.items()}.get(language.lower())
    ...
    if sample_rate != SAMPLE_RATE:
        if not USE_TORCHAUDIO:
            raise NotImplementedError(
                "Resampling not available without torchaudio")
