...
def format_whisper_timestamped_response(
    ...
    return {
        ...
        "confidence-score": (
            round(np.exp(np.array([r["avg_logprob"]
                  for r in segments])).mean(), 2)
            if len(segments)
            else 0.0
        ),
        "words": words,
...
def format_faster_whisper_response(
        ...
        if segment.words:
            ...
            for word in words:
                ...
                word["confidence"] = round(
                    np.mean([c for c in word["confidence"]]), 2)
    ...
    transcription = {
        ...
        "language": language,
        "confidence": round(
            np.exp(np.mean([segment["avg_logprob"]
                   for segment in segments_list])), 2
        ),
