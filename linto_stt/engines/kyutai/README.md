...
> See the [main README](https://github.com/linto-ai/linto-stt/blob/master/README.md) for the WebSocket protocol and general docs.
...
```bash
LOG_TRANSCRIPTS=true LOG_LEVEL=INFO FINAL_TRANSCRIPT_DELAY=1.5 STREAMING_PORT=8002 KYUTAI_URL=ws://localhost:8080 uv run main.py -m websocket -e kyutai
```
...
USE_SEMANTIC_VAD=false uv run main.py -m websocket -e kyutai
```
...
The container exposes the same `/streaming` endpoint as other LinTO engines and
...
After `git submodule update --init --recursive` inside linto_stt/engines/kyutai/delayed-streams-modeling
