...
from linto_stt.engines.whisper.stt import (
    logger,
    USE_CTRANSLATE2,
...
async def wssDecode(ws: WebSocketServerProtocol, model_and_alignementmodel):
    ...
    try:
        ...
        if USE_CTRANSLATE2:
            ...
            asr = FasterWhisperASR(model=model, lan=language, beam_size=DEFAULT_BEAM_SIZE,
                                   best_of=DEFAULT_BEST_OF, temperature=DEFAULT_TEMPERATURE)
        else:
            logger.info("Using whisper_timestamped for decoding")
            asr = WhisperTimestampedASR(model=model, lan=language, beam_size=DEFAULT_BEAM_SIZE,
                                        best_of=DEFAULT_BEST_OF, temperature=DEFAULT_TEMPERATURE)
        ...
        while True:
            ...
            if message is None:
                silence_chunk = np.zeros(
                    int(sample_rate * received_chunk_size * 1), dtype=np.float32)
                off = online.buffer_time_offset
                dur = len(online.audio_buffer)/online.sampling_rate
                online.insert_audio_chunk(silence_chunk)
                logger.debug(
                    f"Silence chunk inserted ({(len(silence_chunk)/online.sampling_rate):.2f}s) at {off:.2f} for {dur:.2f} (now {(len(online.audio_buffer)/online.sampling_rate):.2f})")
            else:
                if len(pile) < 2:
                    pile.append(message)
            ...
            if online.get_buffer_size() >= STREAMING_MIN_CHUNK_SIZE:
                if current_task and not current_task.done():
                    ...
                    if len(pile) > 0:
                        logger.debug(
                            f"Launching new task t={(len(online.audio_buffer)/online.sampling_rate)+online.buffer_time_offset:.2f}s")
                        current_task = asyncio.get_event_loop().run_in_executor(
                            executor, online.process_iter)
                        pile.pop(0)
            else:
                logger.debug(
                    f"Chunk too small {online.get_buffer_size()}<{STREAMING_MIN_CHUNK_SIZE} (added {len(audio_chunk)/sample_rate}), skipping")
...
class OnlineASRProcessor:
    ...
    def transcribe(self, audio, conversion_function):
        ...
        formatted_words = self.asr.ts_words(
            res, conversion_function if self.vad else None)
    ...
    def chunk_completed_segment(self, res, chunk_silence=False, speech_segments=None):
        ...
        #     return
        # deactivated option - allow to set a limit to the buffer size, over this limit it will cut on last commited word instead of segments
        self.buffer_trimming_words = None
...
class FasterWhisperASR(ASRBase):
    ...
    def __init__(self, lan, model=None, logfile=sys.stderr, condition_on_previous_text=None, beam_size=None, best_of=None, temperature=None):
        ...
        self.hotwords = sanitize_prompt(default_hotwords)
        ...
        for segment in segments:
            for word in segment.words:
                ...
                if timestamps_convert_function is not None:
                    start, end = timestamps_convert_function(
                        word.start, word.end)
                    t = (start, end, w)
                else:
