...
class Http_Server:
    def __init__(self, name: str, model, use_gpu, decode, load_wave_buffer, warmup):
        ...
        self.router.add_api_route(
            "/healthcheck", self.healthcheck, methods=["GET"])
        self.router.add_api_route(
            "/transcribe", self.transcribe, methods=["POST"])
    ...
    def transcribe(self, request: Request, file: UploadFile):
        try:
            ...
            if not file.filename:
                raise ValueError(
                    f"No audio file was uploaded (missing 'file' key)")
...
def main(model, use_gpu, decode, load_wave_buffer, warmup):
    app = FastAPI()
    server = Http_Server("LinTO-STT", model, use_gpu,
                         decode, load_wave_buffer, warmup)
