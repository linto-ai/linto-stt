...
def read_envdefault(project_root: str, backend: str) -> dict:
    """Parse a backend's .envdefault file into a dict using python-dotenv."""
    env_path = os.path.join(project_root, "linto_stt", "backends", backend, ".envdefault")
...
def build_env_dict(project_root: str, backend: str, overrides: dict = None) -> dict:
    """Merge .envdefault with overrides, removing SERVICE_MODE (set by CLI)."""
    env = read_envdefault(project_root, backend)
